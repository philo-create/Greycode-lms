...
with open('/app/applet/src/app/dashboard/admin/curriculum/page.tsx', 'r') as f:
    content = f.read()

# Update fetch logic to use grade? It already fetches by schoolId and returns Record<string, LessonStatus>. Wait, our new service `fetchLessonStatuses` returns Record<string, LessonStatus> for a given grade if grade is provided. If not, it returns all for the school, but we might have overlapping lesson_ids across grades if they aren't uniquely named?
# Actually lesson_ids are like `R-T1-W1`, `1-T1-W1`, so they are unique across grades! Thus, fetching all for a school into a Record is perfectly safe.

# We just need to change the status check from 'teacher_prepared' to 'pending_approval'
# and 'teacher_unlocked' to 'teacher_unlocked', etc.
content = content.replace("'teacher_prepared'", "'pending_approval'")
content = content.replace("await updateLessonStatus(selectedSchool.id, lessonId, 'student_unlocked');", "await updateLessonStatus(selectedSchool.id, grade, lessonId, 'unlocked_for_students');")
content = content.replace("await updateLessonStatus(selectedSchool.id, nextLesson.id, 'teacher_unlocked');", "await updateLessonStatus(selectedSchool.id, grade, nextLesson.id, 'teacher_unlocked');")
content = content.replace("status === 'student_unlocked'", "status === 'unlocked_for_students'")

# We need to make sure we pass 'grade' to updateLessonStatus. In the mapping, `grade` is available.
# Let's verify `handleApprove` definition.
target_approve = """  const handleApprove = async (lessonId: string) => {
    if (!selectedSchool) return;
    
    // Update current lesson to student_unlocked
    await updateLessonStatus(selectedSchool.id, grade, lessonId, 'unlocked_for_students');
    
    // Also unlock the next lesson for teachers
...
# Oops, `handleApprove` is defined outside the grade mapping loop. Let's rewrite `handleApprove` to accept `grade`
target_approve_old = """  const handleApprove = async (lessonId: string) => {
    if (!selectedSchool) return;
...
# Wait, `replace` above already corrupted it probably because `grade` wasn't defined in that scope. Let's do a regex or just overwrite the file entirely.

with open('/app/applet/src/app/dashboard/admin/curriculum/page.tsx', 'w') as f:
    f.write("""'use client';
